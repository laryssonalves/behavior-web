...
export class StudentService {
  ...
  getStudentList(): Observable<Student[]> {
    return this.httpClient.get<Student[]>(this.studentUrl).pipe(
      map(students => students.map(student => Student.createFromJSON(student)))
    )
  }
  ...
  deleteStudent(pk: string | number) {
    ...
    this.httpClient.delete(studentDetailUrl).subscribe(
      ...
      error => {
        this.nbToastrService.danger(null, 'Não foi possível deletar o Estudante')
      }
    )
  }
